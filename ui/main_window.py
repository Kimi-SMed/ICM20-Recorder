...
class MainWindow(QMainWindow):
    ...
    def _do_stop_recording(self) -> None:
        self._bridge.submit_coro(self._ble.stop_recording())
        if self._writer:
            self._writer.close()
            self._writer = None
    ...
    # ------------------------------------------------------------------
    # BLE callbacks (called from asyncio thread)
    # ------------------------------------------------------------------
    ...
    def _on_handshake_done(self) -> None:
        ...
        self._start_btn.setEnabled(True)
        self._status_bar.showMessage("Handshake complete. Ready to record.")
